...
// Effects (byte 6). The effect is intentionally rendered locally by each board: a single
// beat cue can therefore animate dozens of RGBIC pixels without filling the CAN bus with
// per-pixel updates. PWM-only zones use the same timing/envelope but ignore the cue colour.
...
#define SHOW_FX_CHASE         4   // several narrow travelling bands; high-energy sections
#define SHOW_FX_RIPPLE        5   // expanding waves from the centre; drops and bass hits
